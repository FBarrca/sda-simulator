...
def summarize_policy(spec: PolicySpec, result: SimulationResult) -> PolicySummary:
    return PolicySummary(
        name=spec.name,
        mean_total_cost=result.metric("total_cost").mean(),
...
def evaluate_policy_set(
    ...
    results = {spec.name: evaluate_policy(spec) for spec in policies}
...
def save_policy_comparison(
    ...
    colors = {
        "Nearest feasible": "#3a86ff",
        "Priority deadline": "#2a9d8f",
        "Risk aware": "#e76f51",
    }
    ...
    fig.text(
        0.5,
        0.915,
        "500 shared bootstrap futures, 28 simulated days, Spanish warehouse-to-customer lanes.",
        ha="center",
        fontsize=10,
        color="#4a4a4a",
    )
...
def _plot_backlog_paths(ax, results, colors) -> None:
    for name in ("Nearest feasible", "Priority deadline", "Risk aware"):
...
def main() -> None:
    ...
    output_path = save_policy_comparison(output=args.output)
